/**
 * ═══════════════════════════════════════════════════════════════════════════
 * 【EG-2 归档资产 · 不参与构建】src/_archived/ —— tsconfig exclude 隔离位
 * ═══════════════════════════════════════════════════════════════════════════
 * 本文件 = 旧 4 主动技效果结算层（active-skill/active-skill-effects.ts）在 B2 批次
 * （R2-6 旧 4 技退役）时的**完整收档快照**（逐行拷贝，未修改）。
 * - 归档裁决：EG-2「旧 4 主动技/超武/进化链代码退役方式：归档不删」；
 * - 替代实现：src/active-skill/derivative/derivative-skills.ts（8 衍生技，CC 走状态层）；
 * - 保留引擎：ActiveSkill 控制器（CD/充能/100ms 防抖/埋点）与 active-skill-math.ts 纯数学
 *   仍在 src/ 且被现有运行时/测试引用——迁移期原文件保留，B5 开局重写后原文件本体退役时
 *   以本快照为准收档；
 * - 对应归档用例：tests/archived/active-skill-v1-effects.test.ts（vitest include 外，不收集）。
 * ═══════════════════════════════════════════════════════════════════════════
 *
