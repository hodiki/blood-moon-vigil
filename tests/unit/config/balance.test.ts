...
describe('balance 数值常量表与 GDD 一致（test-framework §4 埋点断言基线）', () => {
  ...
  it('玩家初始属性（upgrade-pool §③ / E1-S6 验收）', () => {
    expect(PLAYER.MOVE_SPEED).toBe(220);
...
describe('敌人面板常量与 enemies §③ 一致（E2-S2 基线）', () => {
  it('僵尸 12HP/55/10/1.0/14/1；疾行 10HP/150/8/0.8/12/2；厚血 600HP/35/20/1.5/22/15；Boss 6000HP/28/30/2.0/40/100', () => {
    ...
    expect(ENEMIES.tank).toEqual({ hp: 600, speed: 35, damage: 20, attackInterval: 1.5, radius: 22, xp: 15 });
...
describe('生成器常量与 spawner §③ 一致（E2-S4 基线）', () => {
  it('budget 参数：基数 1.2 / 线性 2.5 / 周期 1200 / 波幅 0.4 / 周期 75s', () => {
    expect(SPAWNER.BASE_BUDGET).toBe(1.2);
    expect(SPAWNER.LINEAR_SCALE).toBe(2.5);
    expect(SPAWNER.LINEAR_TOTAL_SECONDS).toBe(1200);
    expect(SPAWNER.WAVE_AMPLITUDE).toBe(0.4);
    expect(SPAWNER.WAVE_PERIOD_SECONDS).toBe(75);
  });

  it('20:00 Boss 收束 + 同屏节流参数', () => {
